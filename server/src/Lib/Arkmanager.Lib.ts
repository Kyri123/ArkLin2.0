...
export function ConfigToJSON( Content : string ) : Partial<IInstanceData> {
	...
	for ( let Line = 0; Line < ClearInstance.length; ++Line ) {
		const [ DataKey, RawDataValue ] = ClearInstance[ Line ].split( "#" )[ 0 ].trim().split( '=', 2 ) as any[];
		...
		if ( DataKey === "ark_GameModIds" ) {
			DataValue = DataValue.split( "," ).map( E => parseInt( E ) ).filter( e => !isNaN( e ) );
	...
	for ( const [ Key, Value ] of Object.entries( Content ) ) {
		if ( Value === "" ) {
			...
			for ( const [ ExtraKey, ExtraValue ] of Object.entries( Value ) ) {
				Lines.push( `${ Key === "Flags" ? "arkflag_" : "arkopt_" }${ ExtraKey }='${ ExtraValue }'` );
			}
		}
...
export function FillWithDefaultValues( Servername : string, Content : Partial<IInstanceData> ) : IInstanceData {
	if ( global.__PublicIP ) {
		Content.panel_publicip = __PublicIP;
	}

	Content.arkbackupdir = path.join( process.env.APPEND_BASEDIR || "/", __server_backups, Servername );
	Content.arkserverroot = path.join( process.env.APPEND_BASEDIR || "/", __server_dir, Servername );
	Content.logdir = path.join( process.env.APPEND_BASEDIR || "/", __server_logs, Servername );
	Content.arkStagingDir = path.join( process.env.APPEND_BASEDIR || "/", __server_backups, "Staging" );
