...
export class SSHLib {
	...
	async Init() {
		...
		this.SSHConnection.connect( {
			...
			.then( () => {
				this.HasInit = true;
				SystemLib.Log( "Connected to SSH as", ConfigManager.GetDashboardConifg.SSH_User );
			} )
	...
	async Exec( command : string, parameters : string[], options? : SSHExecOptions & {
		stream? : 'stdout' | 'stderr';
	} ) {
		if ( !this.HasInit ) {
			return "ERROR";
		}
		return await this.SSHConnection.exec( command, parameters, options ).catch( e => {
			this.CrashCount++;
	...
	async ExecCommandInScreen( ScreenName : string, command : string, options? : SSHExecCommandOptions | undefined ) : Promise<SSHExecCommandResponse> {
		return await this.ExecCommand( [ "screen", '-dmS', ScreenName, 'bash', '-c', `'${ command }' && exit` ].join( " " ), options );
	}
}