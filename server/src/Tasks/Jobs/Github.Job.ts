...
export default new JobTask( ConfigManager.GetTaskConfig.GithubQueryInterval, "Github", async() => {
	...
	try {
		...
		if ( Releases.data.length > 0 ) {
			...
			await DB_GithubReleases.create( Releases.data.map<IGithubReleases>( E => ( {
			...
			if ( Current ) {
				...
				if ( TestNext ) {
					...
					SSHManager.ExecCommandInScreen( "ArkLinUpdate", `${ SHUpdateFile } ${ Branch }` ).then( () => {
					} );
				}
			}
		}
