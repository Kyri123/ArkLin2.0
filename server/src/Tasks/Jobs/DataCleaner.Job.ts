...
export default new JobTask( ConfigManager.GetTaskConfig.DataCleanerInterval, "DataCleaner", async() => {
	// Clear old Sessions
	SystemLib.DebugLog( "[TASKS] Running Task", SystemLib.ToBashColor( "Red" ), "DataCleaner" );
	...
	for await ( const Instance of DB_Instances.find() ) {
		...
		if ( !Server.Init() || !fs.existsSync( Server.InstanceConfigFile ) ) {
			SystemLib.LogWarning( "[DB] Cleanup old DB information (Not Exsisting):", Data.Instance );
