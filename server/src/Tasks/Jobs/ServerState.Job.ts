...
export default new JobTask( ConfigManager.GetTaskConfig.ServerStateInterval, "ServerState", async() => {
	SystemLib.DebugLog( "[TASKS] Running Task", SystemLib.ToBashColor( "Red" ), "ServerState" );
	...
	for ( const Instance of fs.readdirSync( InstancesFolder ) ) {
		if ( Instance.endsWith( ".cfg" ) ) {
			...
			const FilePath = path.join( __server_arkmanager, "instances", `${ Instance }` );
	...
	if ( global.__PublicIP ) {
		...
		for ( const [ InstanceName, InstanceData ] of Object.entries( ServerData ) ) {
			...
			if ( await ServerL.Init() ) {
				...
				const VersionFile = path.join( __server_dir, InstanceName, "version.txt" );
				const ServerExecFile = path.join( __server_dir, InstanceName, InstanceData.arkserverexec );
				const ActionPIDFile = path.join( __server_dir, InstanceName, `.${ InstanceName }.panel.pid` );
				const ArkServerPIDFile = path.join( __server_dir, InstanceName, "ShooterGame/Saved", `.arkserver-${ InstanceName }.pid` );

				const IsInstalled = fs.existsSync( ServerExecFile );

				if ( fs.existsSync( ActionPIDFile ) ) {
					InstanceState.ArkmanagerPID = parseInt( fs.readFileSync( ActionPIDFile ).toString() );
					if ( InstanceState.ArkmanagerPID !== 0 ) {
						try {
							const Result = await SSHManager.ExecCommand( `ps -fp ${ InstanceState.ArkmanagerPID.toString() }`, { encoding: 'utf-8' } );
							if ( Result.code === 1 ) {
				...
				if ( IsInstalled ) {
					...
					if ( fs.existsSync( VersionFile ) ) {
						InstanceState.ServerVersion = fs.readFileSync( VersionFile ).toString().replaceAll( " ", "" ).trim();
					}

					if ( fs.existsSync( ArkServerPIDFile ) ) {
						InstanceState.ArkserverPID = parseInt( fs.readFileSync( ArkServerPIDFile ).toString() );
						if ( InstanceState.ArkserverPID !== 0 ) {
							try {
								const Result = await SSHManager.ExecCommand( `ps -fp ${ InstanceState.ArkserverPID.toString() }`, { encoding: 'utf-8' } );
								if ( Result.code === 1 ) {
					...
					if ( InstanceState.ArkserverPID !== 0 ) {
						InstanceState.IsListen = await IsServerOnline( Number( InstanceData.ark_QueryPort ), ConfigManager.GetTaskConfig.ServerStateInterval / Count );
						InstanceState.State = InstanceState.IsListen ? "Online" : "Running";
				...
				const Background = `/img/backgrounds/${ ServerL.GetConfig().serverMap }.jpg`;

				await ServerL.SetServerState( InstanceState, {
					LOGO: fs.existsSync( path.join( __basedir, "public", Icon ) ) ? Icon : `/img/maps/TheIsland.jpg`,
					BG: fs.existsSync( path.join( __basedir, "public", Background ) ) ? Background : `/img/maps/TheIsland.jpg`,
				} );
