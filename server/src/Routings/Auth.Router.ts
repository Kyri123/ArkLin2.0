...
export default function( Api : core.Express ) {
			...
			try {
				...
				if ( UserDB ) {
					if ( !UserDB.permissions ) {
						UserDB = ( await DB_Accounts.findByIdAndUpdate( User._id, { permissions: [] } ) )!;
					}

					if ( User.permissions !== UserDB.permissions && User.exp ) {
						const NewToken = GenerateAccessToken( UserDB.toJSON(), ( User.exp - Math.trunc( Date.now() / 1000 ) ) / 24 / 60 / 60 );
						response.json( {
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		const Message : IAPIResponseMessage = {
			AlertType: "danger",
			Message: "Account konnte nicht erstellt werden da die daten nicht richtig sind.",
		...
		if ( Request.password && Request.email && Request.user && Request.passwordagain && Request.accountkey ) {
			const State : boolean[] = [
				Request.user.length < 6,
				!( /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test( Request.email ) ),
				Request.password.length < 7,
				Request.passwordagain !== Request.password || Request.passwordagain.length < 7,
				...
				if ( Key !== null ) {
					...
					if ( AccountCheck === null ) {
						...
						if ( NewAccount !== null ) {
							...
							Message.Message = "Account wurde erstellt du wirst nun eingeloggt.";
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.login && Request.password && Request.stayloggedin !== undefined ) {
			...
			if ( UserCheck !== null ) {
				...
				Response.Auth = true;
				Response.Data = {
					JsonWebToken: GenerateAccessToken( UserCheck.toJSON(), Request.stayloggedin ? ConfigManager.GetApiConfig.AccountHashExpire : 1 )
				}
			}
		}
