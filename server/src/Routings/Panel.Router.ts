...
export default function( Api : core.Express ) {
	...
	Api.get( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.UserClass.HasPermission( EPerm.PanelLog ) ) {
			if ( fs.existsSync( __LogFile ) ) {
				Response.Data = fs.readFileSync( __LogFile ).toString().split( "\n" ).reverse();
			}
		}
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.UserClass.HasPermission( EPerm.ManagePanel ) ) {
			SystemLib.LogWarning( "User Request:", SystemLib.ToBashColor( "Red" ), "RESTART" )
			process.exit( 0 );
		}
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.UserClass.HasPermission( EPerm.PanelSettings ) && Request.Config && Request.Data ) {
	...
	Api.get( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.UserClass.HasPermission( EPerm.PanelSettings ) && Request.Config ) {
