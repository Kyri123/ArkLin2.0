...
export default function( Api : core.Express ) {
	...
	Api.get( Url, ( async( request : Request, response : Response ) => {
		...
		for ( const InstanceData of Object.values( await Request.UserClass.GetAllServerWithPermission() ) ) {
			Total++;
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.UserClass.HasPermission( EPerm.ManageServers ) && Request.Config ) {
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.UserClass.HasPermission( EPerm.ManageServers ) && Request.InstanceName ) {
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.Command && Request.ServerInstance && Request.Parameter ) {
			if ( Request.UserClass.HasPermission( EPerm.ManageServers ) && Request.UserClass.HasPermissionForServer( Request.ServerInstance ) ) {
				const Server = new ServerLib( Request.ServerInstance );
				if ( await Server.Init() ) {
					Response.Success = await Server.ExecuteCommand( Request.Command, Request.Parameter );
					Response.Message = {
						AlertType: "success",
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.ServerInstance && Request.UserClass.HasPermissionForServer( Request.ServerInstance ) ) {
			const Server = new ServerLib( Request.ServerInstance );
			if ( await Server.Init() ) {
				if ( Request.LogFile ) {
	...
	SystemLib.Log( "Install Router", SystemLib.ToBashColor( "Red" ), Url, SystemLib.ToBashColor( "Default" ), "| Mode:", SystemLib.ToBashColor( "Red" ), "POST" )
	Api.post( Url, ( async( request : Request, response : Response ) => {
		const Response : IAPIResponseBase<Record<string, string> | {
	...
	Api.post( Url, ( async( request : Request, response : Response ) => {
		...
		if ( Request.UserClass.HasPermission( EPerm.ManageServers ) && Request.ServerInstance && Request.ConfigFile && typeof Request.ConfigContent === "object" ) {
			const Server = new ServerLib( Request.ServerInstance );
			if ( await Server.Init() ) {
				if ( await Server.SetServerConfig( Request.ConfigFile, Request.ConfigContent ) ) {
					Response.Success = true;
					Response.Message = {
						AlertType: "success",
