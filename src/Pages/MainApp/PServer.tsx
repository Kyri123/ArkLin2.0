...
export default function PServer() {
	...
	if ( !Server.IsValid() || !Account.Account.HasPermissionForServer( InstanceName as string ) ) {
		...
		<>
			...
			<Row>
				<Col>
					<Suspense fallback={ <></> }>
						<Routes>
							<Route path={ `/logs` } element={ <SPServerLogs InstanceName={ Instance }/> }/>
							<Route path={ `/mods` } element={ <SPServerMods InstanceName={ Instance }/> }/>
							<Route path={ `/konfiguration` } element={ <SPServerConfig InstanceName={ Instance }/> }/>
							<Route path={ "*" } element={ <Navigate to={ `/server/${ InstanceName }/logs` }/> }/>
						</Routes>
					</Suspense>
				</Col>
			</Row>
