		...
		<>
			<div className="row" id="serverControlCenterContainer">
				{ Object.entries( InstanceData ).map( ( [ InstanceName ] ) => (
					<CServerAdminCard InstanceName={ InstanceName } key={ "SERVERADMIN" + InstanceName }/>
				) ) }

				<span className="col-lg-6 col-xl-4">
					<div onClick={ () => setShowNewServer( true ) }
			...
			<Modal size={ "lg" } show={ ShowNewServer } onHide={ () => setShowNewServer( false ) }>
				...
				<Modal.Body>
					{ Object.entries( FormData ).map( ( [ Key, Value ], Idx ) => (
						<CLTEInput
							...
							key={ "NEWSERVER" + Key + Idx } Value={ Value } OnValueSet={ ( Val ) => {
							console.log( Val );
				...
				<Modal.Footer>
					<LTELoadingButton BtnColor={ "success" } IsLoading={ IsSending } onClick={ CreateServer }>
						<FontAwesomeIcon icon={ "save" }/> Erstellen
					</LTELoadingButton>
					<LTELoadingButton BtnColor={ "danger" } onClick={ () => setShowNewServer( false ) }>
