...
function PreLoginApp() {
	...
	return (
		<div className="register-page bg-image" style={ {
			...
			<Suspense fallback={ <></> }>
				<Modal onHide={ () => {
				} } show={ true } centered backdrop={ false } contentClassName={ "rounded-4" }>
					<Modal.Header className={ "p-5 pb-4 border-bottom-0" }>
						<h1 className="fw-bold mb-0 fs-2">KAdmin ArkLin 2.0</h1>
						<LTERibbon>
							Alpha
						</LTERibbon>
					</Modal.Header>
					<Modal.Body className={ "p-5 pt-0" }>
						...
						<Row>
							<Col span={ 6 }>
								<Link to="https://discord.gg/uXxsqXD" target="_blank"
									  className={ "btn btn-secondary w-100 mb-2 rounded-3" }>
									<FontAwesomeIcon icon={ [ "fab", "discord" ] } className={ "pe-2" }/>
									Discord
								</Link>
							</Col>
							<Col span={ 6 }>
								<Link to="https://github.com/Kyri123/ArkLin2.0/" target="_blank"
									  className={ "btn btn-secondary w-100 mb-2 rounded-3" }>
									<FontAwesomeIcon icon={ [ "fab", "github" ] } className={ "pe-2" }/>
									Github
								</Link>
							</Col>
						</Row>
						<Link to={ IsLogin ? "/signup" : "/signin" }
