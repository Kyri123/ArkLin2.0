...
const options = [
	...
	{ value: EArkmanagerCommands.cancelshutdown, label: 'Runterfahren abbrechen' },
...
export default function CServerAction( Props : {
	...
	const [ ParameterMask, setParameterMask ] = useState<Record<string, ISelectMask[]>>( { para: [] } );
	const [ Selected, setSelected ] = useState<{ value : EArkmanagerCommands; label : string; } | null>( null );
	...
	const SendCommand = async() => {
		if ( Selected ) {
			setIsSending( true );
			const Response = await API_ServerLib.SendCommandToServer( Props.InstanceName, Selected.value, Parameter.filter( C => C.trim().replaceAll( " ", "" ) !== "" ) );
	...
	return (
		<Modal onHide={ Props.OnClose } show={ Props.Show && State.ArkmanagerPID === 0 } centered size={ "lg" }>
			...
			<Modal.Body>
				...
				{ ( ParameterMask.para.length > 0 && Selected !== null ) &&
					<>
						<hr/>
						<CLTEInput Value={ Parameter } OnValueSet={ setParameter } SelectMask={ ParameterMask }
								   ValueKey={ "para" }> Parameter </CLTEInput>
					</>
				}
			</Modal.Body>
			<Modal.Footer>
				<LTELoadingButton onClick={ SendCommand } Disabled={ Selected === null } IsLoading={ IsSending }>
