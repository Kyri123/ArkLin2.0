...
export default function TopNavigation( Props: {
	...
	const restartPanel = async() => {
		...
		if( accept?.isConfirmed ) {
			const result = await apiAuth.panelAdmin.restart.mutate();
			if( result ) {
	...
	const updatePanel = async() => {
		...
		if( accept?.isConfirmed ) {
			const result = await apiAuth.panelAdmin.update.mutate();
			if( result ) {
				fireSwalFromApi( result, true );
			}
		}
			...
			<div className="container-fluid bg-gray-200 p-2 border-bottom">
				<div className="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start p-2">
						...
						{ user.hasPermission( EPerm.PanelSettings ) && (
							<div className="dropdown text-end ms-2">
								...
								<div className="dropdown-menu dropdown-menu-lg dropdown-menu-left">
									...
									<button onClick={ () => Props.showLog( true ) }
					...
					<div className="dropdown text-end ms-3 me-2">
						...
						<div className="dropdown-menu flex-column flex-lg-row align-items-stretch justify-content-start p-3 rounded-3 shadow-lg"
							data-bs-theme="light"
							style={ { minWidth: 300 } }>
